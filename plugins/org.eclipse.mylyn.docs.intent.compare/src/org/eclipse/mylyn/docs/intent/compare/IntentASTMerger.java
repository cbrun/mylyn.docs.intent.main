...
public class IntentASTMerger {
	...
	public void mergeFromLocalToRepository(EObject localRoot, EObject repositoryRoot) throws MergingException {
		...
		// ResourceImpl(URI.createURI("http://mysampleuri.com"));
		// sampleResource.getContents().add(localRoot);
		//
		// // Step 0.2 Defining a scope provider
		// MatchModel match = null;
		// IMatchScopeProvider scopeProvider = new
		// IntentScopeProvider(localRoot, repositoryRoot);
		// Map<String, Object> optionsMap = new HashMap<String, Object>();
		// optionsMap.put(MatchOptions.OPTION_MATCH_SCOPE_PROVIDER,
		// scopeProvider);
		//
		// // Step 1 : matching the local and the repository root using a custom
		// MatcheEngine.
		// match = new IntentMatchEngine(localRoot,
		// repositoryRoot).contentMatch(localRoot, repositoryRoot,
		// optionsMap);
		//
